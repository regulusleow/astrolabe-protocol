...
export function parseReleaseArgs(args) {
  if (args.length !== 1) {
    throw new Error("Usage: release-prepare.mjs <SemVer>");
  }
...
export function prepareRelease({
  ...
  requireCleanBranch(commandRunner);
  ...
  runRequired(commandRunner, "./gradlew", [
  ...
  runRequired(commandRunner, "git", ["tag", "-a", version, "-m", `Astrolabe Protocol ${version}`]);

  return { previousVersion, version, tag: version };
}

function requireCleanBranch(commandRunner) {
  ...
  if (!branch) throw new Error("Cannot prepare a release from a detached HEAD");
...
if (resolve(process.argv[1] ?? "") === scriptPath) {
  try {
    ...
    process.stdout.write(`Prepared Astrolabe Protocol ${result.version}: commit and tag created but not pushed\n`);
  } catch (error) {
