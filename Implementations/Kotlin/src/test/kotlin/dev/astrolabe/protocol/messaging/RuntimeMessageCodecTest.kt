...
class RuntimeMessageCodecTest {
    ...
    fun `request envelope preserves method parameters and extensions`() {
        val source = fixture("v2/Fixtures/valid/unknown-method-request.json")
    ...
    fun `response envelope enforces status member exclusivity`() {
        val success = codec.decodeResponse(fixture("v2/Fixtures/valid/handshake-response.json"))
        val failure = codec.decodeResponse(fixture("v2/Fixtures/valid/node-detail-failure-response.json"))
        ...
        assertFailsWith<RuntimeMessageException.InvalidEnvelope> {
            codec.decodeResponse(fixture("v2/Fixtures/invalid/success-response-contains-error.json"))
        }
        assertFailsWith<RuntimeMessageException.InvalidEnvelope> {
            codec.decodeResponse(fixture("v2/Fixtures/invalid/failure-response-contains-payload.json"))
        }
    }
    ...
    fun `codec rejects unsupported protocol versions and scalar roots`() {
        assertFailsWith<RuntimeMessageException.UnsupportedProtocolVersion> {
            codec.decodeRequest(fixture("v2/Fixtures/invalid/request-uses-v1.json"))
        }
    ...
    fun `codec decodes standalone valid attribute values`() {
        val document = codec.decodeDocument(fixture("v2/Fixtures/valid/vector-attribute-value.json"))

        assertIs<JsonObject>(document)
    }

    private fun validFixtureNames(): List<String> = checkNotNull(
        javaClass.classLoader.getResource("v2/Fixtures/valid")
