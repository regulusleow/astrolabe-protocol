...
class RuntimeHierarchyModelTest {
    ...
    fun `hierarchy fixtures decode through typed contracts`() {
        val request = codec.decodeRequest(fixture("v2/Fixtures/valid/hierarchy-snapshot-request.json"))
        val response = codec.decodeResponse(fixture("v2/Fixtures/valid/hierarchy-snapshot-response.json"))
    ...
    fun `hierarchy values reject missing units and inconsistent visibility`() {
        assertFailsWith<RuntimeMessageException.InvalidDocument> {
            codec.decodeValue(
                fixture("v2/Fixtures/invalid/coordinate-rect-missing-unit.json"),
                RuntimeCoordinateRect.serializer()
            )
        }
        assertFailsWith<RuntimeMessageException.InvalidDocument> {
            codec.decodeValue(
                fixture("v2/Fixtures/invalid/inconsistent-visibility.json"),
