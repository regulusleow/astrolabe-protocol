...
class RuntimePatchingModelTest {
    ...
    fun `patch identifiers and patterns reject ambiguous values`() {
        val request = codec.decodeRequest(fixture("v2/Fixtures/invalid/attribute-identifier-not-namespaced.json"))
        ...
        assertFailsWith<RuntimeMessageException.InvalidDocument> {
            codec.decodeValue(
                fixture("v2/Fixtures/invalid/patch-pattern-not-namespaced.json"),
                RuntimePatchableAttribute.serializer()
            )
        }
        assertFailsWith<RuntimeMessageException.InvalidDocument> {
            val unsafeRequest = codec.decodeRequest(
                fixture("v2/Fixtures/invalid/unsafe-integer-attribute-value.json")
            )
    ...
    private fun <Parameters, Payload> decodeRoundTrip(
        ...
        val request = codec.decodeRequest(fixture("v2/Fixtures/valid/$fixturePrefix-request.json"))
        val response = codec.decodeResponse(fixture("v2/Fixtures/valid/$fixturePrefix-response.json"))
