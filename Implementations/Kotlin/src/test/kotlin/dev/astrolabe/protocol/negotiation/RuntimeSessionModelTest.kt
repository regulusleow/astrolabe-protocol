...
class RuntimeSessionModelTest {
    ...
    fun `handshake request decodes typed parameters`() {
        val request = codec.decodeRequest(fixture("v2/Fixtures/valid/handshake-request.json"))
    ...
    fun `handshake response decodes typed payload`() {
        val response = codec.decodeResponse(fixture("v2/Fixtures/valid/handshake-response.json"))
    ...
    fun `typed handshake values round trip without exposing JSON assembly`() {
        val sourceRequest = codec.decodeRequest(fixture("v2/Fixtures/valid/handshake-request.json"))
        ...
        val sourceResponse = codec.decodeResponse(fixture("v2/Fixtures/valid/handshake-response.json"))
    ...
    fun `protocol range rejects descending and cross-major values`() {
        ...
        assertFailsWith<RuntimeMessageException.InvalidDocument> {
            codec.decodeRequest(fixture("v2/Fixtures/invalid/descending-protocol-range.json")).let { request ->
    ...
    fun `typed decoding rejects a mismatched method before reading payload`() {
        val request = codec.decodeRequest(fixture("v2/Fixtures/valid/application-info-request.json"))
        val response = codec.decodeResponse(fixture("v2/Fixtures/valid/application-info-response.json"))
    ...
    fun `error codes and extension keys follow shared identifier rules`() {
        ...
        assertFailsWith<RuntimeMessageException.InvalidDocument> {
            codec.decodeValue(
                fixture("v2/Fixtures/invalid/extension-key-not-namespaced.json"),
